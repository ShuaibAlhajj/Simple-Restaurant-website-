...
## 2024-05-25 - [Character Counters and Form Feedback]
**Learning:** For inputs with character limits, providing a real-time counter improves UX by preventing unexpected truncation. Using `aria-live="status"` or `aria-live="polite"` ensures updates are announced to screen readers. For dark-themed apps, ensure counters have sufficient contrast (e.g., `rgba(255, 255, 255, 0.7)`) and are manually reset if the form is cleared programmatically.
**Action:** Always include character counters for length-limited textareas and link them using `aria-describedby` for accessibility.
