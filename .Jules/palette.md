...
## 2024-05-25 - [Real-time Form Feedback and Character Limits]
**Learning:** For textareas with constraints, combining the native `maxlength` attribute with a real-time JavaScript counter improves UX by preventing silent truncation. Linking the counter via `aria-describedby` ensures screen reader users are aware of the limit and current state upon focus.
**Action:** Always pair `maxlength` with a visual counter and ensure the counter is initialized on page load and reset on form submission.
