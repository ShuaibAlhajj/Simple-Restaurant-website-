...
## 2024-05-25 - [Real-time Character Counters and Accessible Forms]
**Learning:** For forms with constraints, a real-time character counter reduces cognitive load by letting users know how much space they have left before they hit a hard limit. Linking the counter via `aria-describedby` and using `aria-live="polite"` provides accessible, non-intrusive feedback.
**Action:** Always pair `maxlength` with a visual counter and ensure it resets alongside the form fields to maintain UI consistency.
