...
## 2025-01-24 - [Character Counter and Dynamic Modal Accessibility]
**Learning:** Combining native `maxlength` with a JavaScript-driven character counter improves both usability and accessibility, especially when linked via `aria-describedby` and `aria-live`. For dynamic modals, updating `alt` text and context-specific `aria-labels` (e.g., "Add [Item] to Order") ensures screen reader users have the same context as visual users.
**Action:** Always link status counters to their inputs using `aria-describedby` and ensure dynamic modal actions include specific item names in their labels.
