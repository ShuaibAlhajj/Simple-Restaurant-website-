...
## 2024-06-26 - [Character Counter Layout and Contrast]
**Learning:** For character counters in forms, placing them within a flexbox container (`.form-help`) alongside error messages and using `margin-left: auto` ensures they remain correctly positioned even when errors are toggled. On dark themes, using reduced opacity white (e.g., `rgba(255, 255, 255, 0.7)`) provides sufficient contrast without being visually overwhelming.
**Action:** Always group auxiliary input information (errors, counters) in a flexible helper container and ensure high-contrast but secondary visual priority for counters.
