...
## 2024-05-25 - [Form Accessibility: Character Counters and Persistence]
**Learning:** When adding character limits to textareas, combining `maxlength` with a visual counter improves UX. For accessibility, the counter must be linked via `aria-describedby`. Crucially, counters must be initialized on page load to handle browser-persisted input (e.g., after a refresh), otherwise the visual state will mismatch the actual input length until the next interaction.
**Action:** Always link counters via `aria-describedby` and initialize their state in `DOMContentLoaded` or equivalent setup.
