...
## 2026-06-13 - [Character Counter with Live Feedback]
**Learning:** For forms on dark backgrounds, using a semi-transparent white (e.g., rgba(255,255,255,0.7)) for secondary info like character counters maintains high contrast while distinguishing it from primary text. Combining `maxlength` for constraint and `aria-live="polite"` for state updates provides a robust, accessible experience.
**Action:** Use `.form-help` containers with flexbox to align error messages and counters, and always reset counter state in the form's success/reset handler.
